var osclasgap_services_resource_call_result;

//function osclasgap_services_resource_call
var osclasgap_services = {
	 ...
	 * options.site_path
	 * 		The full site path (default: osclasgap_settings.site_path)
	 * options.base_path
	 * 		The osclas base path (default: osclasgap_settings.base_path)
	 * options.endpoint
	 * 		The endpoint name (default : osclasgap_settings.services_endpoint_default)
	 * options.type
	 ...
	 * 		post.http://www.osclasgap.org/?q=osclasgap/system/connect.json
	 * options.async
	...
	"resource_call":function (options) {
		...
		osclasgap_services_resource_call_result = null;
		result = null;
		...
		try {
			...
			options = osclasgap_services_resource_get_default_options(options);
			...
			service_resource_call_url = osclasgap_services_resource_url(options);
			
			// Set default local storage key if one wasn't provided.
			if (!options.local_storage_key) {
				options.local_storage_key = osclasgap_services_default_local_storage_key(options.type,options.resource_path);
			}
			
			// If no load_from_local_storage option was set, set the default.
			if (!options.load_from_local_storage) {
				options = osclasgap_services_get_load_from_local_storage_default(options);
			}
			else {
			...
			if (!options.save_to_local_storage) {
				options = osclasgap_services_get_save_to_local_storage_default(options);
			}
			else {
			...
			// Save the current options in osclasgap_services.options for backup.
			...
			if (result) {
			    ...
			    if (options.async == false) {
				    ...
				    if (!result.errorThrown) {
				    	...
				    	osclasgap_services_resource_clean_local_storage_dependencies(options);
				    	
				    }
				    
				    // Save a copy of the service resource call result in the
			    	// global variable in case anybody needs it.
			    	osclasgap_services_resource_call_result = result;
			    	
			    	return osclasgap_services_resource_call_result;
 ...
 * 		The endpoint name (default : osclasgap_settings.services_endpoint_default)
*/
function osclasgap_services_resource_url(options) {
	// Set default values for options if none were provided.
	if (!options.site_path) {
		options.site_path = osclasgap_settings.site_path;
	}
	if (!options.base_path) {
		options.base_path = osclasgap_settings.base_path;
	}
	if (!options.endpoint) {
		options.endpoint = osclasgap_settings.services_endpoint_default;
	}
	return options.site_path + options.base_path +"/" + options.resource_path;
}
 ...
 */
function osclasgap_services_default_local_storage_key(type,resource_path) {
	return type + "." + resource_path;
}

function osclasgap_services_resource_get_default_options(options) {
	// Set default values for options if none were provided.
	if (!options.site_path) {
		options.site_path = osclasgap_settings.site_path;
	}
...
function osclasgap_services_get_load_from_local_storage_default(options) {
	// If no load_from_local_storage option was set, set the default.
	...
	switch (options.type.toLowerCase()) {
		case "get":
			...
			if (options.resource_path.indexOf("node/") != -1) {
				if ($.mobile.activePage.attr('id') == "osclasgap_page_node_edit") {
						options.load_from_local_storage = "0";
				}
			}
			else if (options.resource_path.indexOf("comment/") != -1) {
				if ($.mobile.activePage.attr('id') == "osclasgap_page_comment_edit") {
						options.load_from_local_storage = "0";
				}
			}
			break;
...
function osclasgap_services_get_save_to_local_storage_default(options) {
	switch (options.type.toLowerCase()) {
		case "get":
			break;
		case "post":
...
function osclasgap_services_resource_clean_local_storage_dependencies(options) {
	...
	console.log("osclasgap_services_resource_clean_local_storage_dependencies");
	console.log(JSON.stringify(options));
	switch (options.type.toLowerCase()) {
		case "get":
			break;
		case "post":
			...
			if (
				...
				// system/connect.json
				osclasgap_services_system_connect.local_storage_remove();
				// osclasgap_system/connect.json
				osclasgap_services_resource_system_connect.local_storage_remove();
			}
			// Node create resource.
			else if (options.resource_path.indexOf("node.json") != -1) {
				// Remove views datasource content json.
				views_options = {"path":"views_datasource/osclasgap_content"};
				osclasgap_views_datasource_retrieve.local_storage_remove(views_options);
			}
			// Comment create resource.
			else if (options.resource_path.indexOf("comment.json") != -1) {
				// Remove views datasource comment json.
				views_options = {"path":"views_datasource/osclasgap_comments"};
				osclasgap_views_datasource_retrieve.local_storage_remove(views_options);
				// Remove the comment's node comment views json.
				views_options = {"path":"views_datasource/osclasgap_comments/" + options.nid};
				osclasgap_views_datasource_retrieve.local_storage_remove(views_options);
				// Remove the comment's node.
				osclasgap_services_node_retrieve.local_storage_remove({"nid":options.nid});
			}
			break;
		case "put":
			// User update resource.
			if (options.resource_path.indexOf("user/") != -1) {
				...
				osclasgap_services_system_connect.local_storage_remove();
				// Remove osclasgap_system/connect.json.
				osclasgap_services_resource_system_connect.local_storage_remove();
			}
			// Node update resource.
			else if (options.resource_path.indexOf("node/") != -1) {
			...
			// Comment update resource.
				...
				osclasgap_services_comment_retrieve.local_storage_remove({"cid":options.cid});
				// Remove views datasource comment json.
				views_options = {"path":"views_datasource/osclasgap_comments"};
				osclasgap_views_datasource_retrieve.local_storage_remove(views_options);
